...
func main() {
	...
	fmt.Printf("netloom-agent ready for node policy and dataplane reconciliation endpoint=%s entries=%d allow=%s deny=%s tcx=%s\n", result.EndpointID, result.Entries, result.Allowed, result.Denied, result.TCX)
}
