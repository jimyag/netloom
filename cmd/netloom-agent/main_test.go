...
func TestRunPolicyStatusExportWithStoreReportsFilteredJSON(t *testing.T) {
	...
	podALastEvent := time.Date(2026, 7, 17, 1, 5, 0, 0, time.UTC)
	podBLastEvent := time.Date(2026, 7, 17, 0, 5, 0, 0, time.UTC)
	lastEventCutoff := time.Date(2026, 7, 17, 0, 30, 0, 0, time.UTC)
	if err := store.Save(t.Context(), policyStatusDocument{
		...
		Statuses: []dataplane.PolicyEndpointStatus{{
			...
			LastEvent:        dataplane.PolicyUpdateEvent{EndpointID: endpointID, Revision: 7, Success: true, OccurredAt: &podALastEvent},
			HasLastEvent:     true,
		}, {
	...
	out.Reset()
	if err := runPolicyStatusExportWithStore(t.Context(), policyStatusExportOptions{lastEventRuleCookie: "43"}, &out, store); err != nil {
		t.Fatal(err)
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-rule-cookie filtered policy-status-export output: %v\n%s", err, out.String())
	}
	if got.FilterLastEventRuleCookie != 43 || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
		t.Fatalf("last-event-rule-cookie filtered statuses = %+v, want only pod-b with cookie 43", got)
	}

	out.Reset()
	if err := runPolicyStatusExportWithStore(t.Context(), policyStatusExportOptions{lastEventRuleRef: "prod/db/allow-db"}, &out, store); err != nil {
		t.Fatal(err)
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-rule-ref filtered policy-status-export output: %v\n%s", err, out.String())
	}
	...
	out.Reset()
	if err := runPolicyStatusExportWithStore(t.Context(), policyStatusExportOptions{lastEventRemediation: string(dataplane.PolicyMapOverflowClear)}, &out, store); err != nil {
		t.Fatal(err)
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-remediation filtered policy-status-export output: %v\n%s", err, out.String())
	}
	...
	out.Reset()
	if err := runPolicyStatusExportWithStore(t.Context(), policyStatusExportOptions{lastEventErrorContains: "capacity exceeded"}, &out, store); err != nil {
		t.Fatal(err)
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-error-contains filtered policy-status-export output: %v\n%s", err, out.String())
	}
	if got.FilterLastEventErrorContains != "capacity exceeded" || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
		t.Fatalf("last-event-error-contains filtered statuses = %+v, want only capacity failure pod-b", got)
	}

	out.Reset()
	if err := runPolicyStatusExportWithStore(t.Context(), policyStatusExportOptions{lastEventOccurredAfter: lastEventCutoff.Format(time.RFC3339)}, &out, store); err != nil {
		t.Fatal(err)
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-occurred-after filtered policy-status-export output: %v\n%s", err, out.String())
	}
	if got.FilterLastEventOccurredAfter == nil || !got.FilterLastEventOccurredAfter.Equal(lastEventCutoff) || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != endpointID {
		t.Fatalf("last-event-occurred-after filtered statuses = %+v, want only pod-a after cutoff", got)
	}

	out.Reset()
	if err := runPolicyStatusExportWithStore(t.Context(), policyStatusExportOptions{lastEventOccurredBefore: lastEventCutoff.Format(time.RFC3339)}, &out, store); err != nil {
		t.Fatal(err)
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-occurred-before filtered policy-status-export output: %v\n%s", err, out.String())
	}
	if got.FilterLastEventOccurredBefore == nil || !got.FilterLastEventOccurredBefore.Equal(lastEventCutoff) || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
		t.Fatalf("last-event-occurred-before filtered statuses = %+v, want only pod-b before cutoff", got)
	}
...
func TestRunPolicyRolloutHistoryClearWithStoreReportsJSON(t *testing.T) {
	firstCompleted := time.Date(2026, 7, 19, 1, 0, 0, 0, time.UTC)
	secondCompleted := time.Date(2026, 7, 19, 1, 10, 0, 0, time.UTC)
	thirdCompleted := time.Date(2026, 7, 19, 1, 20, 0, 0, time.UTC)
	completedCutoff := time.Date(2026, 7, 19, 1, 15, 0, 0, time.UTC)
	history := []policyRolloutHistoryEntry{
		{ID: "1", Source: "manual", Name: "canary", Node: "node-a", Store: "ebpf", CompletedAt: firstCompleted, Rollout: agent.PolicyEndpointRollout{Planned: 1, Applied: 1}},
		{ID: "2", Source: "desired-state", Name: "nightly", Node: "node-a", Store: "ebpf", CompletedAt: secondCompleted, Rollout: agent.PolicyEndpointRollout{Planned: 2, Applied: 1, Failed: 1}},
		{ID: "3", Source: "manual", Name: "canary", Node: "node-b", Store: "ebpf", CompletedAt: thirdCompleted, Rollout: agent.PolicyEndpointRollout{Planned: 1, Applied: 1}},
	}
	raw, err := json.Marshal(history)
	if err != nil {
		t.Fatal(err)
	...
	store := ovsdbPolicyRolloutHistoryStore{syncer: syncer}
	var out bytes.Buffer
	err = runPolicyRolloutHistoryClearWithStore(t.Context(), policyRolloutHistoryClearOptions{
		source:          "manual",
		name:            "canary",
		completedBefore: completedCutoff.Format(time.RFC3339),
	}, &out, store)
	if err != nil {
		t.Fatal(err)
	}
	var got policyRolloutHistoryClearOutput
	if err := json.Unmarshal(out.Bytes(), &got); err != nil {
		t.Fatalf("decode policy-rollout-history-clear output: %v\n%s", err, out.String())
	}
	if !got.Ready || got.TotalEvents != 3 || got.ClearedEvents != 1 || got.RemainingEvents != 2 || got.FilterSource != "manual" || got.FilterName != "canary" || got.FilterCompletedBefore == nil || !got.FilterCompletedBefore.Equal(completedCutoff) {
		t.Fatalf("output = %+v, want filtered rollout history clear metadata", got)
	}
	if len(got.Cleared) != 1 || got.Cleared[0].ID != "1" {
		t.Fatalf("cleared = %+v, want rollout 1", got.Cleared)
	}
	var remaining []policyRolloutHistoryEntry
	if err := json.Unmarshal([]byte(syncer.values[policyRolloutHistoryKey]), &remaining); err != nil {
		t.Fatalf("decode remaining rollout history: %v", err)
	}
	if len(remaining) != 2 || remaining[0].ID != "2" || remaining[1].ID != "3" {
		t.Fatalf("remaining = %+v, want rollouts 2 and 3", remaining)
	}

	out.Reset()
	err = runPolicyRolloutHistoryClearWithStore(t.Context(), policyRolloutHistoryClearOptions{}, &out, store)
	if err == nil || !strings.Contains(err.Error(), "requires -all or at least one filter") {
		t.Fatalf("err = %v, want required selector", err)
	}
}
...
func TestPolicyEndpointAPIRolloutHistoryClearsFilteredHistory(t *testing.T) {
	firstCompleted := time.Date(2026, 7, 19, 1, 0, 0, 0, time.UTC)
	secondCompleted := time.Date(2026, 7, 19, 1, 10, 0, 0, time.UTC)
	thirdCompleted := time.Date(2026, 7, 19, 1, 20, 0, 0, time.UTC)
	completedCutoff := time.Date(2026, 7, 19, 1, 15, 0, 0, time.UTC)
	...
	metrics := newAgentMetrics(dataplane.NewInMemoryPolicyStore())
	if err := configurePolicyRolloutHistory(t.Context(), metrics, ovsdbPolicyRolloutHistoryStore{syncer: syncer}); err != nil {
		t.Fatal(err)
	}

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodDelete, "/policy/endpoints/rollout/history?source=manual&name=canary&completed_before="+url.QueryEscape(completedCutoff.Format(time.RFC3339)), nil)
	metrics.handlePolicyEndpoints(recorder, request)
	if recorder.Code != http.StatusOK {
		t.Fatalf("clear rollout history status = %d, body=%s", recorder.Code, recorder.Body.String())
	}
	var output policyRolloutHistoryClearOutput
	if err := json.Unmarshal(recorder.Body.Bytes(), &output); err != nil {
		t.Fatalf("decode clear rollout history response: %v\n%s", err, recorder.Body.String())
	}
	if !output.Ready || output.TotalEvents != 3 || output.ClearedEvents != 1 || output.RemainingEvents != 2 || output.FilterSource != "manual" || output.FilterName != "canary" || output.FilterCompletedBefore == nil || !output.FilterCompletedBefore.Equal(completedCutoff) {
		t.Fatalf("clear output = %+v, want filtered rollout history metadata", output)
	}
	if len(output.Cleared) != 1 || output.Cleared[0].ID != "1" {
		t.Fatalf("cleared = %+v, want rollout 1", output.Cleared)
	}
	if history := metrics.policyRolloutHistory(); len(history) != 2 || history[0].ID != "2" || history[1].ID != "3" {
		t.Fatalf("memory rollout history = %+v, want rollouts 2 and 3", history)
	}
	var persisted []policyRolloutHistoryEntry
	if err := json.Unmarshal([]byte(syncer.values[policyRolloutHistoryKey]), &persisted); err != nil {
		t.Fatalf("decode persisted rollout history: %v", err)
	}
	if len(persisted) != 2 || persisted[0].ID != "2" || persisted[1].ID != "3" {
		t.Fatalf("persisted = %+v, want rollouts 2 and 3", persisted)
	}

	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodDelete, "/policy/endpoints/rollout/history", nil)
	metrics.handlePolicyEndpoints(recorder, request)
	if recorder.Code != http.StatusBadRequest {
...
func TestPrintReconcileResultIncludesPolicyMapUsageSummary(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"policy_map_recommended_capacity=17",
		`policy_map_recommended_endpoint="prod\x00pod-a"`,
		...
		"provider_network_status=physnet-a:ready:1/1:0:none:tenants=1:subnets=1:endpoints=2:load_balancers=1:nat_rules=1:policy_routes=1:security_groups=1:security_group_rules=2:prod=ok:1/2:2/3:1/4:1/5:1/6:1/7:2/8,physnet-b:degraded:0/1:1:type-mismatch",
...
func TestPolicyEventsAPIFiltersByCapacityHotspotRuleRef(t *testing.T) {
	...
	got = policyEventsOutput{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode pressure-min policy events response: %v\n%s", err, recorder.Body.String())
	}
	if got.FilterPressureMinPercent != 100 || got.EventCount != 1 || len(got.Events) != 1 {
		t.Fatalf("pressure-min filtered events = %+v, want one saturated overflow event", got)
	}
...
func TestPolicyEventsAPIRejectsInvalidPressureThreshold(t *testing.T) {
	metrics := newAgentMetrics(dataplane.NewInMemoryPolicyStore())
	observeAgentReconcileResult(metrics, agent.ReconcileResult{Node: "node-a"}, "memory", time.Millisecond)

	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodGet, "/policy/events?pressure_min_percent=bad", nil)
	metrics.handlePolicyEvents(recorder, request)
...
func TestRunPolicyEventsWithStoreReportsFilteredJSON(t *testing.T) {
	...
	stdout.Reset()
	if err := runPolicyEventsWithStore(t.Context(), policyEventsOptions{pressureMinPercent: "90", limit: 10}, &stdout, store); err != nil {
		t.Fatal(err)
	}
	got = policyEventsOutput{}
	if err := json.Unmarshal(stdout.Bytes(), &got); err != nil {
		t.Fatalf("decode pressure-min filtered policy-events output: %v\n%s", err, stdout.String())
	}
	if got.FilterPressureMinPercent != 90 || got.EventCount != 1 || len(got.Events) != 1 || got.Events[0].Revision != 2 {
		t.Fatalf("pressure-min filtered events = %+v, want pod-a deny-ssh revision 2", got)
	}
	if got.Events[0].PolicyMapPressurePercent != 90 {
		t.Fatalf("event pressure percent = %d, want 90", got.Events[0].PolicyMapPressurePercent)
	}

	stdout.Reset()
	if err := runPolicyEventsWithStore(t.Context(), policyEventsOptions{recommendedCapacityMin: "12", limit: 10}, &stdout, store); err != nil {
		t.Fatal(err)
	}
	got = policyEventsOutput{}
	if err := json.Unmarshal(stdout.Bytes(), &got); err != nil {
		t.Fatalf("decode recommended-capacity filtered policy-events output: %v\n%s", err, stdout.String())
	}
	if got.FilterRecommendedCapacityMin != 12 || got.EventCount != 1 || len(got.Events) != 1 || got.Events[0].Revision != 2 {
		t.Fatalf("recommended-capacity filtered events = %+v, want pod-a deny-ssh revision 2", got)
	}
...
func TestPolicyEndpointAPIReportsLifecycleStatus(t *testing.T) {
	...
	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/policy/endpoints?last_event_rule_cookie=43", nil)
	metrics.handlePolicyEndpoints(recorder, request)

	if recorder.Code != http.StatusOK {
		t.Fatalf("last-event-rule-cookie filter status = %d, body=%s", recorder.Code, recorder.Body.String())
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-rule-cookie policy endpoint API response: %v\n%s", err, recorder.Body.String())
	}
	if got.FilterLastEventRuleCookie != 43 || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
		t.Fatalf("last-event-rule-cookie filtered statuses = %+v, want pod-b with cookie 43", got)
	}

	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/policy/endpoints?last_event_rule_ref=prod/db/allow-db", nil)
	metrics.handlePolicyEndpoints(recorder, request)

	if recorder.Code != http.StatusOK {
		t.Fatalf("last-event-rule-ref filter status = %d, body=%s", recorder.Code, recorder.Body.String())
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-rule-ref policy endpoint API response: %v\n%s", err, recorder.Body.String())
	}
	if got.FilterLastEventRuleRef != "prod/db/allow-db" || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
	...
	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/policy/endpoints?last_event_remediation=clear", nil)
	metrics.handlePolicyEndpoints(recorder, request)

	if recorder.Code != http.StatusOK {
		t.Fatalf("last-event-remediation filter status = %d, body=%s", recorder.Code, recorder.Body.String())
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-remediation policy endpoint API response: %v\n%s", err, recorder.Body.String())
	}
	if got.FilterLastEventRemediation != string(dataplane.PolicyMapOverflowClear) || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
		t.Fatalf("last-event-remediation filtered statuses = %+v, want pod-b with clear remediation", got)
	}
	...
	if recorder.Code != http.StatusOK {
		t.Fatalf("last-event-error-contains filter status = %d, body=%s", recorder.Code, recorder.Body.String())
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-error-contains policy endpoint API response: %v\n%s", err, recorder.Body.String())
	}
	if got.FilterLastEventErrorContains != "capacity exceeded" || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
		t.Fatalf("last-event-error-contains filtered statuses = %+v, want pod-b with capacity failure", got)
	}

	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/policy/endpoints?last_event_occurred_after="+url.QueryEscape(lastEventCutoff.Format(time.RFC3339)), nil)
	metrics.handlePolicyEndpoints(recorder, request)

	if recorder.Code != http.StatusOK {
		t.Fatalf("last-event-occurred-after filter status = %d, body=%s", recorder.Code, recorder.Body.String())
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-occurred-after policy endpoint API response: %v\n%s", err, recorder.Body.String())
	}
	if got.FilterLastEventOccurredAfter == nil || !got.FilterLastEventOccurredAfter.Equal(lastEventCutoff) || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != endpointID {
		t.Fatalf("last-event-occurred-after filtered statuses = %+v, want pod-a after cutoff", got)
	}

	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/policy/endpoints?last_event_occurred_before="+url.QueryEscape(lastEventCutoff.Format(time.RFC3339)), nil)
	metrics.handlePolicyEndpoints(recorder, request)

	if recorder.Code != http.StatusOK {
		t.Fatalf("last-event-occurred-before filter status = %d, body=%s", recorder.Code, recorder.Body.String())
	}
	got = policyStatusOutput{}
	if err := json.Unmarshal(recorder.Body.Bytes(), &got); err != nil {
		t.Fatalf("decode last-event-occurred-before policy endpoint API response: %v\n%s", err, recorder.Body.String())
	}
	if got.FilterLastEventOccurredBefore == nil || !got.FilterLastEventOccurredBefore.Equal(lastEventCutoff) || got.EndpointCount != 1 || len(got.Statuses) != 1 || got.Statuses[0].EndpointID != model.EndpointKey("prod", "pod-b") {
		t.Fatalf("last-event-occurred-before filtered statuses = %+v, want pod-b before cutoff", got)
	}
	...
	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/policy/endpoints?last_event_rule_cookie=bad", nil)
	metrics.handlePolicyEndpoints(recorder, request)
	...
	recorder = httptest.NewRecorder()
	request = httptest.NewRequest(http.MethodGet, "/policy/endpoints?last_event_occurred_before=bad", nil)
	metrics.handlePolicyEndpoints(recorder, request)

	if recorder.Code != http.StatusBadRequest {
		t.Fatalf("invalid last-event-occurred-before status = %d, want 400; body=%s", recorder.Code, recorder.Body.String())
	}
	if !strings.Contains(recorder.Body.String(), "invalid last_event_occurred_before") {
		t.Fatalf("body missing invalid last_event_occurred_before error: %s", recorder.Body.String())
	}
...
func TestPolicyEndpointAPIRolloutUsesPressureAwareBatchSize(t *testing.T) {
	...
	if got.Rollout.PressureRecommendedCapacity != 12 || got.Rollout.PressureRecommendedEndpoint != model.EndpointKey("prod", "pod-a") {
...
func TestAgentMetricsExportsLatestPolicyAndTCXCounters(t *testing.T) {
	...
	observeAgentReconcileResult(metrics, agent.ReconcileResult{
		...
		PolicyEndpointStatus: []dataplane.PolicyEndpointStatus{{
			...
			LastEvent: dataplane.PolicyUpdateEvent{
				...
				Stats:            dataplane.PolicyUpdateStats{Added: 2, Updated: 3, Deleted: 4, Unchanged: 5},
				RuleCookies:      []uint32{7, 8},
				RuleRefs:         []string{"prod/web/deny-client", "prod/web/allow-api"},
				CapacityHotspots: []dataplane.PolicyMapCapacityHotspot{{RuleRef: "prod/web/deny-client", Entries: 9}, {RuleRef: "prod/web/allow-api", Entries: 4}},
				Success:          true,
				Remediated:       true,
				Remediation:      string(dataplane.PolicyMapOverflowClear),
				FailureReason:    dataplane.PolicyUpdateFailureCapacityExceeded,
			},
			HasLastEvent: true,
			LastSeen:     &lastSeen,
		...
		ProviderNetworkStatus: []linuxdatapath.ProviderNetworkStatus{
			...
			},
			{
				...
				ReadyLinks:      0,
				IssueCount:      2,
				Reasons:         []string{"ovsdb-port-drift", "parent-down"},
			},
		},
	...
	for _, expected := range []string{
		...
		`netloom_agent_policy_rollout_pressure_max_percent{node="node-a",rollout="prod-rollout",store="ebpf"} 91`,
		`netloom_agent_policy_rollout_pressure_threshold_percent{node="node-a",rollout="prod-rollout",store="ebpf"} 80`,
		`netloom_agent_policy_rollout_pressure_adjusted{node="node-a",rollout="prod-rollout",store="ebpf"} 1`,
		...
		`netloom_agent_provider_degraded{node="node-a",store="ebpf"} 1`,
		`netloom_agent_provider_network_ready{node="node-a",provider_network="physnet-a",store="ebpf"} 1`,
		`netloom_agent_provider_network_links{node="node-a",provider_network="physnet-a",store="ebpf"} 1`,
		`netloom_agent_provider_network_ready_links{node="node-a",provider_network="physnet-a",store="ebpf"} 1`,
		`netloom_agent_provider_network_issues{node="node-a",provider_network="physnet-a",store="ebpf"} 0`,
		`netloom_agent_provider_network_ready{node="node-a",provider_network="physnet-b",store="ebpf"} 0`,
		`netloom_agent_provider_network_links{node="node-a",provider_network="physnet-b",store="ebpf"} 1`,
		`netloom_agent_provider_network_ready_links{node="node-a",provider_network="physnet-b",store="ebpf"} 0`,
		`netloom_agent_provider_network_issues{node="node-a",provider_network="physnet-b",store="ebpf"} 2`,
		`netloom_agent_provider_network_issue_reason{node="node-a",provider_network="physnet-b",reason="ovsdb-port-drift",store="ebpf"} 1`,
		`netloom_agent_provider_network_issue_reason{node="node-a",provider_network="physnet-b",reason="parent-down",store="ebpf"} 1`,
		...
		`netloom_agent_provider_tenant_load_balancers{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 1`,
		`netloom_agent_provider_tenant_nat_rules{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 1`,
		`netloom_agent_provider_tenant_policy_routes{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 1`,
		`netloom_agent_provider_tenant_security_groups{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 1`,
		`netloom_agent_provider_tenant_security_group_rules{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 2`,
		...
		`netloom_agent_provider_tenant_max_load_balancers{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 4`,
		`netloom_agent_provider_tenant_max_nat_rules{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 5`,
		`netloom_agent_provider_tenant_max_policy_routes{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 6`,
		`netloom_agent_provider_tenant_max_security_groups{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 7`,
		`netloom_agent_provider_tenant_max_security_group_rules{node="node-a",provider_network="physnet-a",store="ebpf",tenant="prod"} 8`,
		...
		`netloom_agent_policy_endpoint_last_event_failure_reason{endpoint="prod\x00pod-a",node="node-a",reason="capacity_exceeded",store="ebpf"} 1`,
		...
		`netloom_agent_policy_endpoint_last_event_capacity_hotspots{endpoint="prod\x00pod-a",node="node-a",store="ebpf"} 2`,
		`netloom_agent_policy_endpoint_last_event_capacity_hotspot_entries{endpoint="prod\x00pod-a",node="node-a",rank="1",rule_ref="prod/web/deny-client",store="ebpf"} 9`,
		`netloom_agent_policy_endpoint_last_event_capacity_hotspot_entries{endpoint="prod\x00pod-a",node="node-a",rank="2",rule_ref="prod/web/allow-api",store="ebpf"} 4`,
		`netloom_agent_policy_endpoint_last_event_rule_directions{endpoint="prod\x00pod-a",node="node-a",store="ebpf"} 1`,
		`netloom_agent_policy_endpoint_last_event_rule_actions{endpoint="prod\x00pod-a",node="node-a",store="ebpf"} 1`,
		`netloom_agent_policy_endpoint_last_event_rule_direction{direction="ingress",endpoint="prod\x00pod-a",node="node-a",store="ebpf"} 1`,
		`netloom_agent_policy_endpoint_last_event_rule_action{action="drop",endpoint="prod\x00pod-a",node="node-a",store="ebpf"} 1`,
		...
		`netloom_agent_policy_rollout_history_latest_policy_added_entries{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 5`,
		`netloom_agent_policy_rollout_history_latest_policy_updated_entries{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 1`,
		`netloom_agent_policy_rollout_history_latest_policy_deleted_entries{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 2`,
		`netloom_agent_policy_rollout_history_latest_policy_unchanged_entries{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 3`,
		`netloom_agent_policy_rollout_history_latest_policy_events{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 4`,
		`netloom_agent_policy_rollout_history_latest_policy_failed_events{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 1`,
		`netloom_agent_policy_rollout_history_latest_policy_rollback_events{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 1`,
		`netloom_agent_policy_rollout_history_latest_policy_revision_max{node="node-a",rollout="prod-rollout",source="desired-state",store="ebpf"} 8`,
		...
		`netloom_agent_policy_rule_packets_by_action_direction_total{action="",direction="",node="node-a",store="ebpf"} 2`,
		`netloom_agent_policy_rule_allowed_by_action_direction_total{action="",direction="",node="node-a",store="ebpf"} 2`,
		`netloom_agent_policy_rule_packets_by_action_direction_total{action="drop",direction="ingress",node="node-a",store="ebpf"} 1`,
		`netloom_agent_policy_rule_dropped_by_action_direction_total{action="drop",direction="ingress",node="node-a",store="ebpf"} 1`,
		`netloom_agent_policy_rule_deny_drops_by_action_direction_total{action="drop",direction="ingress",node="node-a",store="ebpf"} 1`,
