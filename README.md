...
- `remote_entities` 采用 Cilium `toEntities`/`fromEntities` 类似语义，支持 `all`、`world`、`cluster` 和 `private`；`all` 会展开为 IPv4/IPv6 默认 CIDR，`world` 表示当前 VPC 子网之外的外部地址集合，`cluster` 会展开为当前 VPC 的子网 CIDR，`private` 会展开为 RFC1918 与 ULA CIDR。
