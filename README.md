...
- eBPF/TCX ACL datapath：支持节点接口和工作负载 veth 上的 IPv4 CIDR+L4/ICMP ACL attach，TCX map 使用 LPM trie 匹配 peer CIDR、TCP/UDP 端口前缀范围以及 ICMP type/code 前缀，workload fast path 会按 ingress/egress 安全组方向投影到对应 TCX attach 点；双栈安全组中 IPv4 规则会进入 TCX fast path，IPv6 安全组 CIDR 仍保留在 policy map/evaluator 路径，不会阻断可加速的 IPv4 子集。
