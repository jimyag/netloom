...
`netloom-dns-observer` 可作为 DNS proxy/sidecar 前置观测入口使用：它从 `base64-lines`、`hex-lines` 或单个 `raw` DNS wire response 输入中解析 answer/authority/additional 区域的 `A`/`AAAA`/`CNAME` 记录；配置 `NETLOOM_OVSDB_ENDPOINT` 或传 `-ovsdb` 时会直接合并写入 `Open_vSwitch.external_ids:netloom_dns_observations`，否则原子合并写入 observations 文件，例如 `netloom-dns-observer -observations /tmp/netloom-dns.json < responses.b64`。
