...
- OVN 风格拓扑后端：把逻辑交换、逻辑路由、策略路由、Gateway 和 NAT 转换为带 `external_ids` 所有权标记的批量 `ovn-nbctl` 事务。
...
- Cilium 风格策略更新：policy map replace 会先计算 add/update/delete/unchanged diff，内存 store 具备事务回滚语义。
- Cilium 风格可观测性：policy evaluator 会记录 allow/drop/conntrack 统计，并为策略拒绝或无匹配 drop 生成 drop event。
