...
overflow reason. Overflow errors and policy update events include bounded,
stable capacity hotspots based on each rule's unique policy-map keys, so
operators can identify the SecurityGroup rules most responsible for map
pressure without scraping the full map. Since the userspace evaluator and TCX
policy model both drop traffic when no policy entry matches, this avoids
preserving stale allows after a failed oversized update. Long-running agents
expose the latest endpoint policy
...
metadata, latest and cumulative low-cardinality action/direction aggregate rule counters, cumulative policy
