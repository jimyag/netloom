...
| Metrics | 已实现 | controller/agent `/metrics`, including per-endpoint policy-map revision, usage, pressure, severity, drift, last-event timestamp/success/remediation/diff stats, last-event rule cookie/ref/action/direction gauges, endpoint action-history action/reason/success/failure gauges, and `last_seen` timestamp gauges | command tests |
