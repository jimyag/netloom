...
| Endpoint action history | 已实现 | agent `GET /policy/endpoints/actions/history`; persists lifecycle actions in local OVS `netloom_policy_endpoint_action_history` when OVSDB is configured | `TestPolicyEndpointAPIPersistsActionHistoryToOpenVSwitchExternalID` |
