...
| Policy update events | 已实现 | agent `/policy/events` JSON API with endpoint/success/remediated/remediation/error-substring/pressure-severity/pressure-percent/recommended-capacity/occurred-at time-window/rule-cookie/rule-ref/capacity-hotspot-rule-ref/direction/action filters, timestamped update events, rule cookie/ref/action/direction attribution, structured policy-map pressure severity/recommended-capacity and capacity hotspots for oversized updates, local OVS `netloom_policy_events` persistence, and `netloom-agent policy-events` for recent endpoint policy-map update events | `TestPolicyEventsAPIReportsRecentEndpointEvents`、`TestPolicyEventsAPIFiltersByCapacityHotspotRuleRef`、`TestPolicyEventsAPIRejectsInvalidPressureSeverity`、`TestPolicyEventsAPIRejectsInvalidPressureThreshold`、`TestPolicyEventsAPIRejectsInvalidOccurredBefore`、`TestPolicyEventsAPIFiltersByDirectionAndAction`、`TestPolicyEventsAPIFiltersFailedEvents`、`TestRunPolicyEventsWithStoreReportsFilteredJSON`、`TestRunPolicyEventsWithStoreFiltersRemediatedEvents`、`TestRunPolicyEventsWithStoreFiltersByDirectionAndAction`、`TestAgentMetricsPersistsPolicyEventsToOpenVSwitchExternalID` |
