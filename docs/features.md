...
| OVN health/audit/maintenance | 已实现 | libovsdb health, endpoint connect status, audit stats, compact/stale hooks, DNS `records` column drift detection, and endpoint DHCP option attachment drift detection through NBCTL/libovsdb audit readers | `go test ./cmd/netloom-controller ./internal/ovn`、`TestAuditManagedObjectsFromNBCTLNormalizesDNSRecords`、`TestAuditManagedObjectsFromNBCTLReportsDNSRecordDrift`、`TestNBCTLExecutorManagedOVNRowsResolvesSwitchPortDHCPOptions`、`TestNBCTLExecutorManagedOVNRowsReportsMissingSwitchPortDHCPOptions` |
