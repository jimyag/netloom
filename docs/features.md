...
| Policy status | 已实现 | `netloom-agent policy-status`, agent `/policy/endpoints` JSON API, local OVS `netloom_policy_endpoint_status` persistence, and `netloom-agent policy-status-export` for endpoint lifecycle status | `TestRunPolicyStatusReportsEndpointLifecycleJSON`、`TestPolicyEndpointAPIReportsLifecycleStatus`、`TestRunPolicyStatusExportWithStoreReportsFilteredJSON`、`TestAgentMetricsPersistsPolicyEndpointStatusToOpenVSwitchExternalID` |
