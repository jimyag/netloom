...
| Policy update events | 已实现 | agent `/policy/events` JSON API with endpoint/success/remediated/remediation/error-substring/rule-cookie/rule-ref/capacity-hotspot-rule-ref/direction/action filters, timestamped update events, rule cookie/ref/action/direction attribution, structured policy-map capacity hotspots for oversized updates, local OVS `netloom_policy_events` persistence, and `netloom-agent policy-events` for recent endpoint policy-map update events | `TestPolicyEventsAPIReportsRecentEndpointEvents`、`TestPolicyEventsAPIFiltersByCapacityHotspotRuleRef`、`TestPolicyEventsAPIFiltersByDirectionAndAction`、`TestPolicyEventsAPIFiltersFailedEvents`、`TestRunPolicyEventsWithStoreReportsFilteredJSON`、`TestRunPolicyEventsWithStoreFiltersRemediatedEvents`、`TestRunPolicyEventsWithStoreFiltersByDirectionAndAction`、`TestAgentMetricsPersistsPolicyEventsToOpenVSwitchExternalID` |
