...
| Metrics | 已实现 | controller/agent `/metrics`, including per-endpoint policy-map revision, usage, pressure, severity, recommended capacity, drift, last-event timestamp/success/remediation/diff stats, last-event capacity-hotspot gauges, last-event rule cookie/ref/action/direction gauges, endpoint action-history action/reason/success/failure gauges, active policy freeze-state endpoint and expiry gauges, durable policy rollout state enabled/load-error/paused/applied/failed gauges, provider network ready/link/issue reason gauges, and `last_seen` timestamp gauges | command tests、`TestAgentMetricsExportsLatestPolicyAndTCXCounters`、`TestAgentMetricsReportsPolicyRolloutStateLoadError` |
