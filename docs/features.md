...
| Endpoint | 已实现 | OVN Logical Switch Port、port security、DHCP attachment，并清理普通 endpoint 端口上的 stale `parent_name`、`peer`、`tag_request` 和 `mirror_rules` 语义列 | `TestLibOVSDBTopologyWriterEnsuresEndpointWithDHCPv4`、`TestLibOVSDBTopologyWriterRepairsEndpointSwitchPortStaleTypeOptionsAndTag`、Docker idempotence e2e |
