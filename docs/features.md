...
| Endpoint action history | 已实现 | agent `GET /policy/endpoints/actions/history` and `netloom-agent policy-action-history`; endpoint/action/reason/success/completed-at time-window/limit filters; Prometheus action/reason/success/failure gauges; persists successful and failed lifecycle actions with stable failure reasons in local OVS `netloom_policy_endpoint_action_history` when OVSDB is configured | `TestPolicyEndpointAPIPersistsActionHistoryToOpenVSwitchExternalID`、`TestPolicyEndpointActionHistoryAPIFiltersEndpointActionAndLimit`、`TestPolicyEndpointActionHistoryRecordsFailedAction`、`TestRunPolicyActionHistoryWithStoreReportsFilteredJSON`、`TestAgentMetricsExportsLatestPolicyAndTCXCounters` |
