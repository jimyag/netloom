...
- desired endpoint policy-map pressure mitigation beyond equivalent CIDR rule compaction, deleting non-desired maps, and automatic scoped policy quarantine, such as rollout-aware partial policy updates
- richer per-endpoint policy lifecycle actions beyond endpoint map reset, forced regenerate/reconcile, scoped quarantine, and unquarantine, such as staged rollout
