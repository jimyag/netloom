...
- typed in-database stale repair actions for every drift category, beyond desired-state orphan cleanup, steady-state unexpected/incomplete row cleanup, LoadBalancer managed option repair, opt-in DB compaction, and the operator-controlled stale maintenance hook
