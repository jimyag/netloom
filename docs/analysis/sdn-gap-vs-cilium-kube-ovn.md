...
- typed in-database stale repair actions for every drift category, beyond desired-state orphan cleanup, steady-state unexpected/incomplete row cleanup, Endpoint Logical Switch Port stale type/options/tag repair, Endpoint DHCP attachment/family repair, LoadBalancer managed option/health-check attachment repair, opt-in DB compaction, and the operator-controlled stale maintenance hook
