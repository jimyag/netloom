...
- typed in-database stale repair actions for every drift category, beyond desired-state orphan cleanup, steady-state unexpected/incomplete row cleanup, core OVN name repair, Logical Router stale-options and Router/Router Port disabled-state repair, Subnet router/localnet parent attachment repair, IPv6 router port RA repair, LoadBalancer router/switch parent attachment repair, Endpoint Logical Switch Port address/port-security/type/options/tag/enabled repair, Endpoint DHCP attachment/family repair, DNS row/switch attachment repair, RouteTable static route row repair, RouteTable static route BFD row repair, PolicyRoute row repair, Gateway router options/external_ids repair, NAT row repair, LoadBalancer managed option/health-check attachment repair, opt-in DB compaction, and the operator-controlled stale maintenance hook
