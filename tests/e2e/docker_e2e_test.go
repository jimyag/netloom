...
func TestDockerMultiNodeLab(t *testing.T) {
	...
	for _, expected := range []string{"endpoint=default\x00selftest-pod", "entries=", "allow=allow", "deny=drop", "policy_allowed=3", "policy_dropped=1", "policy_conntrack=1", "policy_established=1", "policy_logged=3", "rule_stats=", "rule_catalog=", "default/web/allow-https", "default/web/deny-range", "0:p=1,b=64,a=1,d=0,r=0,nm=0,ct=1,est=0,log=0", "drop_events=1", "drop_event_refs=default/web/deny-range", "policy_events=3", "policy_event_refs=default/web/allow-https", "trace_events=4", "trace_event_refs=default/web/allow-https", "tcx=attached:lo:ingress:verdict:pass"} {
