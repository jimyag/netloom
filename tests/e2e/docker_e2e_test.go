...
func TestDockerMultiNodeLab(t *testing.T) {
	...
	for _, service := range []string{"node-a", "node-b", "node-c"} {
		run(t, ctx, "docker", "compose", "-f", composeFile, "exec", "-T", service, "apk", "add", "--no-cache", "iproute2")
	...
	for _, expected := range []string{"endpoint=selftest-pod", "entries=", "allow=allow", "deny=drop", "tcx=attached:lo:ingress:verdict:pass"} {
