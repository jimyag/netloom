...
func (p *Planner) EnsureSubnet(_ context.Context, subnet model.Subnet) error {
	...
	if subnet.ProviderNetwork != "" {
		...
		if subnet.VLAN != 0 {
			...
			p.ops = append(p.ops, Operation{Command: "clear", Args: []string{"logical_switch_port", localnetPort, "tag"}})
		}
	} else {
...
func (p *Planner) EnsurePolicyRoute(_ context.Context, route model.PolicyRoute) error {
	...
	p.ops = append(p.ops,
		Operation{Command: "lr-policy-add", Flags: []string{"--may-exist"}, Args: []string{router, fmt.Sprint(route.Priority), match, string(ovnAction)}},
		tagPolicyRouteOperation(route, match),
	)
	return nil
}
