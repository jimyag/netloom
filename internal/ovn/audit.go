...
func managedAuditNBCTLColumns(table string) []string {
	...
	case "Logical_Router_Policy":
		columns = append(columns, "priority", "match", "action", "options", "chain", "jump_chain", "nexthop", "nexthops", "bfd_sessions")
...
func staleManagedColumnShouldDrift(table, key string) bool {
	...
	case "Logical_Router_Port":
		switch key {
		case "dhcp_relay", "ipv6_prefix", "enabled", "options", "gateway_chassis", "ha_chassis_group", "peer":
			return true
		default:
			return false
		}
	case "Logical_Router_Policy":
		switch key {
		case "options", "chain", "jump_chain", "nexthop", "nexthops", "bfd_sessions":
			return true
		default:
			return false
		}
