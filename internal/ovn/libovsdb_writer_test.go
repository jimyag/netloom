...
func TestLibOVSDBTopologyWriterCleanupRepairsReferencedEndpointDHCPDriftInSteadyState(t *testing.T) {
	ctx := context.Background()
	client, closeFn := newTestOVNNBClient(t)
	defer closeFn()
	...
	requireEventually(t, func() bool {
		ports = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalSwitchPort) bool { return row.Name == logicalPort("prod", "pod-a") }).List(ctx, &ports); err != nil || len(ports) != 1 || ports[0].Dhcpv4Options == nil {
			return false
		}
		dhcpRows = nil
		err := client.WhereCache(func(row *ovnnb.DHCPOptions) bool { return row.UUID == *ports[0].Dhcpv4Options }).List(ctx, &dhcpRows)
		return err == nil && len(dhcpRows) == 1
	})
	dhcpUUID := dhcpRows[0].UUID
	dhcpRows[0].Cidr = "10.99.0.0/24"
	dhcpRows[0].Options = map[string]string{"lease_time": "60"}
	dhcpRows[0].ExternalIDs = map[string]string{
		"netloom_owner":       "netloom",
		"netloom_vpc":         "wrong",
		"netloom_endpoint":    "wrong",
		"netloom_dhcp_family": "6",
	}
	updateOps, err := client.Where(&dhcpRows[0]).Update(&dhcpRows[0], &dhcpRows[0].Cidr, &dhcpRows[0].Options, &dhcpRows[0].ExternalIDs)
	if err != nil {
		t.Fatal(err)
	}
	results, err := client.Transact(ctx, updateOps...)
	...
	if opErrors, err := ovsdb.CheckOperationResults(results, updateOps); err != nil {
		t.Fatalf("seed referenced DHCP drift operation errors=%+v: %v", opErrors, err)
	}
	requireEventually(t, func() bool {
		dhcpRows = nil
		err := client.WhereCache(func(row *ovnnb.DHCPOptions) bool { return row.UUID == dhcpUUID }).List(ctx, &dhcpRows)
		return err == nil && len(dhcpRows) == 1 && dhcpRows[0].ExternalIDs["netloom_vpc"] == "wrong"
	})
	...
	requireEventually(t, func() bool {
		ports = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalSwitchPort) bool { return row.Name == logicalPort("prod", "pod-a") }).List(ctx, &ports); err != nil || len(ports) != 1 || ports[0].Dhcpv4Options == nil || *ports[0].Dhcpv4Options != dhcpUUID {
			return false
		}
		dhcpRows = nil
		if err := client.WhereCache(func(row *ovnnb.DHCPOptions) bool { return row.UUID == dhcpUUID }).List(ctx, &dhcpRows); err != nil || len(dhcpRows) != 1 {
			return false
		}
		return dhcpRows[0].Cidr == "10.10.0.0/24" &&
			dhcpRows[0].Options["lease_time"] == "7200" &&
			dhcpRows[0].Options["mtu"] == "1400" &&
			dhcpRows[0].ExternalIDs["netloom_vpc"] == "prod" &&
			dhcpRows[0].ExternalIDs["netloom_endpoint"] == endpointExternalID("prod", "pod-a") &&
			dhcpRows[0].ExternalIDs["netloom_dhcp_family"] == "4"
	})
	stats := writer.LastCleanupStats()
	if stats.FirstReconcileGC || stats.Operations == 0 {
		t.Fatalf("cleanup stats = %+v, want steady-state referenced DHCP repair operations", stats)
	}
}
...
func TestLibOVSDBTopologyWriterCleanupRepairsReferencedNATRowDriftInSteadyState(t *testing.T) {
	ctx := context.Background()
	client, closeFn := newTestOVNNBClient(t)
	defer closeFn()
	...
	requireEventually(t, func() bool {
		routers = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalRouter) bool { return row.Name == logicalRouter("prod") }).List(ctx, &routers); err != nil || len(routers) != 1 || len(routers[0].Nat) != 1 {
			return false
		}
		nats = nil
		err := client.WhereCache(func(row *ovnnb.NAT) bool { return row.UUID == routers[0].Nat[0] }).List(ctx, &nats)
		return err == nil && len(nats) == 1
	})
	natUUID := nats[0].UUID
	nats[0].LogicalIP = "10.99.0.0/24"
	nats[0].ExternalIDs = map[string]string{
		"netloom_owner": "netloom",
		"netloom_vpc":   "wrong",
		"netloom_nat":   "wrong",
	}
	updateOps, err := client.Where(&nats[0]).Update(&nats[0], &nats[0].LogicalIP, &nats[0].ExternalIDs)
	if err != nil {
		t.Fatal(err)
	}
	results, err := client.Transact(ctx, updateOps...)
	...
	if opErrors, err := ovsdb.CheckOperationResults(results, updateOps); err != nil {
		t.Fatalf("seed referenced NAT drift operation errors=%+v: %v", opErrors, err)
	}
	requireEventually(t, func() bool {
		nats = nil
		err := client.WhereCache(func(row *ovnnb.NAT) bool { return row.UUID == natUUID }).List(ctx, &nats)
		return err == nil && len(nats) == 1 && nats[0].ExternalIDs["netloom_vpc"] == "wrong" && nats[0].LogicalIP == "10.99.0.0/24"
	})
	...
	requireEventually(t, func() bool {
		routers = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalRouter) bool { return row.Name == logicalRouter("prod") }).List(ctx, &routers); err != nil || len(routers) != 1 || len(routers[0].Nat) != 1 || routers[0].Nat[0] != natUUID {
			return false
		}
		nats = nil
		if err := client.WhereCache(func(row *ovnnb.NAT) bool { return row.UUID == natUUID }).List(ctx, &nats); err != nil || len(nats) != 1 {
			return false
		}
		return nats[0].LogicalIP == "10.10.0.0/24" &&
			nats[0].ExternalIDs["netloom_vpc"] == "prod" &&
			nats[0].ExternalIDs["netloom_nat"] == "egress"
	})
	stats := writer.LastCleanupStats()
	if stats.FirstReconcileGC || stats.Operations == 0 {
		t.Fatalf("cleanup stats = %+v, want steady-state referenced NAT repair operations", stats)
	}
}
...
func TestLibOVSDBTopologyWriterCleanupRepairsReferencedPolicyRouteDriftInSteadyState(t *testing.T) {
	ctx := context.Background()
	client, closeFn := newTestOVNNBClient(t)
	defer closeFn()
	...
	requireEventually(t, func() bool {
		...
		policies = nil
		err := client.WhereCache(func(row *ovnnb.LogicalRouterPolicy) bool { return row.UUID == routers[0].Policies[0] }).List(ctx, &policies)
		return err == nil && len(policies) == 1
	})
	policyUUID := policies[0].UUID
	policies[0].ExternalIDs = map[string]string{
		"netloom_owner":        "netloom",
		"netloom_vpc":          "wrong",
		"netloom_policy_route": "wrong",
		"netloom_action":       "wrong",
	}
	updateOps, err := client.Where(&policies[0]).Update(&policies[0], &policies[0].ExternalIDs)
	if err != nil {
		t.Fatal(err)
	}
	results, err := client.Transact(ctx, updateOps...)
	...
	if opErrors, err := ovsdb.CheckOperationResults(results, updateOps); err != nil {
		t.Fatalf("seed referenced policy route drift operation errors=%+v: %v", opErrors, err)
	}
	requireEventually(t, func() bool {
		policies = nil
		err := client.WhereCache(func(row *ovnnb.LogicalRouterPolicy) bool { return row.UUID == policyUUID }).List(ctx, &policies)
		return err == nil && len(policies) == 1 && policies[0].ExternalIDs["netloom_vpc"] == "wrong"
	})
	...
	requireEventually(t, func() bool {
		routers = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalRouter) bool { return row.Name == logicalRouter("prod") }).List(ctx, &routers); err != nil || len(routers) != 1 || len(routers[0].Policies) != 1 || routers[0].Policies[0] != policyUUID {
			return false
		}
		policies = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalRouterPolicy) bool { return row.UUID == policyUUID }).List(ctx, &policies); err != nil || len(policies) != 1 {
			return false
		}
		return policies[0].ExternalIDs["netloom_vpc"] == "prod" &&
			policies[0].ExternalIDs["netloom_policy_route"] == "via-fw" &&
			policies[0].ExternalIDs["netloom_action"] == string(model.ActionReroute) &&
			policies[0].Action == ovnnb.LogicalRouterPolicyActionReroute &&
			policies[0].Nexthop != nil &&
			*policies[0].Nexthop == "10.10.0.254"
	})
	stats := writer.LastCleanupStats()
	if stats.FirstReconcileGC || stats.Operations == 0 {
		t.Fatalf("cleanup stats = %+v, want steady-state referenced policy route repair operations", stats)
	}
}

func TestLibOVSDBTopologyWriterCleanupRepairsReferencedStaticRouteDriftInSteadyState(t *testing.T) {
	ctx := context.Background()
	client, closeFn := newTestOVNNBClient(t)
	defer closeFn()
	...
	requireEventually(t, func() bool {
		routers = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalRouter) bool { return row.Name == logicalRouter("prod") }).List(ctx, &routers); err != nil || len(routers) != 1 || len(routers[0].StaticRoutes) != 1 {
			return false
		}
		routes = nil
		err := client.WhereCache(func(row *ovnnb.LogicalRouterStaticRoute) bool { return row.UUID == routers[0].StaticRoutes[0] }).List(ctx, &routes)
		return err == nil && len(routes) == 1
	})
	routeUUID := routes[0].UUID
	routes[0].RouteTable = "legacy"
	routes[0].Options = map[string]string{"ecmp_symmetric_reply": "true"}
	routes[0].ExternalIDs = map[string]string{
		"netloom_owner":       "netloom",
		"netloom_vpc":         "wrong",
		"netloom_route_table": "wrong",
		"netloom_route_key":   "wrong",
	}
	updateOps, err := client.Where(&routes[0]).Update(&routes[0], &routes[0].RouteTable, &routes[0].Options, &routes[0].ExternalIDs)
	if err != nil {
		t.Fatal(err)
	}
	results, err := client.Transact(ctx, updateOps...)
	...
	if opErrors, err := ovsdb.CheckOperationResults(results, updateOps); err != nil {
		t.Fatalf("seed referenced static route drift operation errors=%+v: %v", opErrors, err)
	}
	requireEventually(t, func() bool {
		routes = nil
		err := client.WhereCache(func(row *ovnnb.LogicalRouterStaticRoute) bool { return row.UUID == routeUUID }).List(ctx, &routes)
		return err == nil && len(routes) == 1 && routes[0].ExternalIDs["netloom_vpc"] == "wrong" && routes[0].RouteTable == "legacy"
	})
	...
	requireEventually(t, func() bool {
		routers = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalRouter) bool { return row.Name == logicalRouter("prod") }).List(ctx, &routers); err != nil || len(routers) != 1 || len(routers[0].StaticRoutes) != 1 || routers[0].StaticRoutes[0] != routeUUID {
			return false
		}
		routes = nil
		if err := client.WhereCache(func(row *ovnnb.LogicalRouterStaticRoute) bool { return row.UUID == routeUUID }).List(ctx, &routes); err != nil || len(routes) != 1 {
			return false
		}
		return routes[0].RouteTable == "main" &&
			len(routes[0].Options) == 0 &&
			routes[0].ExternalIDs["netloom_vpc"] == "prod" &&
			routes[0].ExternalIDs["netloom_route_table"] == "main" &&
			routes[0].ExternalIDs["netloom_route_key"] == "10.20.0.0/24|10.10.0.253"
	})
	stats := writer.LastCleanupStats()
	if stats.FirstReconcileGC || stats.Operations == 0 {
		t.Fatalf("cleanup stats = %+v, want steady-state referenced static route repair operations", stats)
	}
}
...
func TestLibOVSDBTopologyWriterCleanupRepairsReferencedLoadBalancerHealthCheckDriftInSteadyState(t *testing.T) {
	ctx := context.Background()
	client, closeFn := newTestOVNNBClient(t)
	defer closeFn()
	...
	if !ok || len(existing.HealthCheck) != 1 {
		t.Fatalf("load balancer row = %+v, want one health check", existing)
	}
	hcUUID := existing.HealthCheck[0]
	var checks []ovnnb.LoadBalancerHealthCheck
	requireEventually(t, func() bool {
		checks = nil
	...
	checks[0].Options = map[string]string{"interval": "99"}
	...
	stats := writer.LastCleanupStats()
	if stats.FirstReconcileGC || stats.Operations == 0 {
		t.Fatalf("cleanup stats = %+v, want steady-state referenced health check repair operations", stats)
	}
}
