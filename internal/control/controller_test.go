...
func TestControllerRejectsInvalidObjectGraph(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantErr: `provider network "physnet-a" tenant "prod" endpoint queues 10 and 11 conflict on endpoint "pod-a" priority 240 overlapping protocol/ports`,
		},
		{
		...
		{
			name: "provider tenant nat rule quota exceeded",
			mutate: func(state *DesiredState) {
				state.ProviderNetworks = []model.ProviderNetwork{{
					Name: "physnet-a",
					Nodes: []model.ProviderNetworkNode{{
				...
				state.NATRules = []model.NATRule{{
					...
					VPC:        "prod",
					Type:       model.ActionDNAT,
					ExternalIP: netip.MustParseAddr("192.0.2.11"),
		...
		{
			name: "duplicate policy rollout",
			mutate: func(state *DesiredState) {
				state.PolicyRollouts = []PolicyRollout{
					{Name: "web-canary", Endpoints: []string{"prod/pod-a"}, BatchSize: 1},
					{Name: "web-canary", Endpoints: []string{"prod/pod-a"}, BatchSize: 1},
				}
			},
			wantErr: "duplicate policy rollout name",
		},
		{
			name: "invalid policy rollout pressure threshold",
			mutate: func(state *DesiredState) {
				state.PolicyRollouts = []PolicyRollout{{
					Name:                     "web-canary",
					Endpoints:                []string{"prod/pod-a"},
					BatchSize:                1,
					PressureThresholdPercent: 101,
				}}
			},
			wantErr: "pressure_threshold_percent must be <= 100",
		},
		{
			name: "invalid policy rollout SLO threshold",
			mutate: func(state *DesiredState) {
				state.PolicyRollouts = []PolicyRollout{{
					Name:                    "web-canary",
					Endpoints:               []string{"prod/pod-a"},
		...
		{
			name: "invalid policy rollout approval callback URL",
			mutate: func(state *DesiredState) {
				state.PolicyRollouts = []PolicyRollout{{
					Name:                "web-canary",
					Endpoints:           []string{"prod/pod-a"},
					...
					Approved:            true,
					ApprovalCallbackURL: "file:///tmp/approval",
				}}
			},
		...
		{
			name: "invalid policy rollout change poll URL",
			mutate: func(state *DesiredState) {
				state.PolicyRollouts = []PolicyRollout{{
					Name:          "web-canary",
					Endpoints:     []string{"prod/pod-a"},
					BatchSize:     1,
					ChangePollURL: "file:///tmp/status",
				}}
			},
			wantErr: "change_poll_url must be http or https",
		},
		{
