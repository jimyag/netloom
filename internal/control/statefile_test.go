...
func TestLoadDesiredStateJSONDecodesSnakeCaseState(t *testing.T) {
	...
	if got := state.ProviderNetworks[0].TenantQuotas[0]; got.Tenant != "prod" || got.MaxSubnets != 2 || got.MaxEndpoints != 10 || got.MaxLoadBalancers != 4 || got.MaxNATRules != 8 || got.MaxPolicyRoutes != 6 {
		t.Fatalf("provider network tenant quota = %+v, want prod 2/10/4/8/6", got)
	}
