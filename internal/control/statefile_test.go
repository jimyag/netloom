...
func TestLoadDesiredStateJSONDecodesSnakeCaseState(t *testing.T) {
	state, err := LoadDesiredStateJSON(strings.NewReader(`{
		"vpcs": [{"name": "prod"}],
		"provider_networks": [{"name": "physnet-a", "isolation": "exclusive", "qos": {"egress_rate_bps": 1000000000, "egress_burst_bps": 64000}, "tenant_quotas": [{"tenant": "prod", "max_subnets": 2, "max_endpoints": 10}], "tenant_queues": [{"tenant": "prod", "queue_id": 10, "min_rate_bps": 100000000, "max_rate_bps": 500000000, "burst_bps": 64000}], "nodes": [{"node": "node-a", "interface": "bond0.100"}, {"node": "node-b", "interfaces": ["ens5", "eth1"]}]}],
	...
	if got := state.ProviderNetworks[0].TenantQueues[0]; got.Tenant != "prod" || got.QueueID != 10 || got.MinRateBPS != 100000000 || got.MaxRateBPS != 500000000 || got.BurstBPS != 64000 {
	...
	if len(state.PolicyRollouts[0].Probes) != 1 || state.PolicyRollouts[0].Probes[0].Name != "web-ready" || state.PolicyRollouts[0].Probes[0].ExpectedStatus != 204 {
